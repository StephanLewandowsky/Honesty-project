...
mask_list = pd.read_csv('/data/honesty/mask_list.csv', header=None)
...
def old_main():
    ...
    for year in tqdm(years, desc='year loop'):
        #fn = Path('.').glob(str(years)+'.txt')
        fn = str(year)+'.txt'
        result = get_matches(fn)
        first_key = list(result.keys())[0]
        length = len(result[first_key])
        result['year'] = [year for i in range(length)]
        result_df = pd.DataFrame.from_dict(result)
        df = df.append(result_df)

    sample_df = pd.DataFrame()
    df['lemma'] = df.terms.apply(get_lemma)
    #if args.freq_count
    df.to_csv(f'result_dict.csv', index=False)
    compute_df(df) 
    #yr_cat = df.groupby(['year', 'category'])
    #for group_name, group in yr_cat:
    #    group_sample = group.sample(10)
    #    sample_df = sample_df.append(group_sample)

    #sample_df.to_csv(f'sampled_snippets.csv', index=False)
##Filter for vocab match
    #for cat in ['truth_seeking', 'belief_speaking', 'seeking_understanding']:
    #     cat_df = pd.read_csv(f'/data/honesty/corpora/NYT/NYT-API/{cat}_df.csv', header=None)                                                           
    #     cat_df.columns = ['vocab', 'freq']
    #     vocab_df = pd.read_csv(f'/data/honesty/{cat}_lemmav2.csv')
    #     vocab_df.columns = ['vocab']
    #     vocab_df.join(cat_df.set_index('vocab'), on='vocab', how='left').to_csv(
    #                        f'/data/honesty/corpora/NYT/NYT-API/{cat}_dfcomb.csv', index=False, header=None)
...
def filter_liwc(args):
    df = pd.read_csv(args.input_file, error_bad_lines=False, warn_bad_lines=True)#, nrows=10000
    df = preprocess(df)
    df.to_csv(f'{args.corpus}_{args.output_file}', index=False)
